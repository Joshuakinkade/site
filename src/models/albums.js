...
export const getAll = () => Album.fetchAll({withRelated: 'coverPhoto'}).then( albums => albums.toJSON());

export const getRecent = ({count}) => {
  let query = Album.query('orderBy','start_date','desc');
  if (count) {
    query.query('limit',count)
  }

  return query.fetchAll({withRelated: 'coverPhoto'}).then( albums => albums.toJSON());
}
